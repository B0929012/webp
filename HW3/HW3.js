...
for(var i=0;i<17;i++){
document.getElementById("gg").innerHTML +=getRandomChar();
}
...
function produce(){
 var x=getRandomChar(); 
 var plus=document.getElementById("gg").innerText
document.getElementById("gg").innerHTML = insertStr(plus,6,x);
}
function disappear(e) {
  var keyCode = (window.event) ? e.which : e.keyCode;
   var del=document.getElementById("gg").innerText;
var n = String.fromCharCode(keyCode);
  if(n==del.substring(del.length-1, del.length)){
document.getElementById("gg").innerHTML=del.substring(0, del.length - 1);
    }
}
